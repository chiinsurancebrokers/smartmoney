...
- Added **📊 Intraday Report** as a first-class navigation area, modeled on the useful thesis-style report from the original app.
- Added deterministic dataset-level Top Signal ranking and confluence context.
- Separated **Put Premium Share** from directional bearish/bullish premium so option type is not confused with intent.
- Added manual Twelve Data market-context refresh for up to 8 symbols and true price/flow divergence detection only when quote data exists.
- Added largest-premium block table with position effect, direction, probable multi-leg structure and Vol/OI context.
- Added sector, inverse-ETF and broad-market cross-confirmation without converting peer ETF premium into fake leveraged notional.
- Added explicit FACT / INFERENCE / RISK guardrails against unsupported claims from the legacy narrative.
- Added dataset-level Claude Analyst → OpenAI Skeptic → Claude Synthesizer with Structured Outputs.
- Dataset AI cannot replace the deterministic Top Signal or invent entry/stop/target levels.
- Added `/api/quotes` manual batch quote endpoint with an 8-symbol free-plan guardrail.
- Added `smoke:report` deterministic regression test.
