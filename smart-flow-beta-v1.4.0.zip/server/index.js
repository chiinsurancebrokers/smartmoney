...
const reportAnalystInstructions=`${reportRules}\nROLE: Dataset-level options-flow analyst. Build a coherent intraday thesis across tickers: strongest signal, real price/flow divergences only where price context exists, cross-confirmation, large-premium evidence and market context. Explain what matters and where the thesis is coherent. In additionalPositions, surface further ETF and stock ideas beyond the top signal so the reader has more than one candidate to evaluate: pick ONLY symbols that already appear in packet.rankedSignals (never invent a ticker), aim for up to 4 ETFs and up to 4 stocks when the packet has that many eligible signals (confidence>=40, coverage>=30, bias not MIXED — relax this only if too few symbols qualify), and give each a one-sentence bilingual rationale grounded in that symbol's own packet fields. Write bilingual fields.`;
...
const reportSynthesisInstructions=`${reportRules}\nROLE: Final editor. Preserve the deterministic top signal and metrics. Reconcile Analyst and Skeptic into a useful intraday report with thesis, supports, risks, monitoring steps and a beginner explanation. Keep and lightly tighten the Analyst's additionalPositions list so the reader still sees several ETF and stock ideas, not only the top signal — every symbol must still come from packet.rankedSignals. No invented trade levels.`;
...
const positionCandidateSchema={type:'object',properties:{
  symbol:{type:'string'},assetType:{type:'string',enum:['ETF','STOCK','UNKNOWN']},bias:{type:'string',enum:['BULLISH','BEARISH','MIXED']},rationale:langTextSchema
},required:['symbol','assetType','bias','rationale'],additionalProperties:false};
const positionCandidateListSchema={type:'array',items:positionCandidateSchema};
const reportAnalystJsonSchema={type:'object',properties:{topSignal:{type:'string'},thesis:langTextSchema,divergences:langListSchema,whaleEvidence:langListSchema,crossConfirmation:langListSchema,supports:langListSchema,caveats:langListSchema,beginner:langTextSchema,additionalPositions:positionCandidateListSchema},required:['topSignal','thesis','divergences','whaleEvidence','crossConfirmation','supports','caveats','beginner','additionalPositions'],additionalProperties:false};
...
const reportSynthesisJsonSchema={type:'object',properties:{headline:langTextSchema,topSignal:{type:'string'},thesis:langTextSchema,supports:langListSchema,risks:langListSchema,monitor:langListSchema,beginner:langTextSchema,additionalPositions:positionCandidateListSchema},required:['headline','topSignal','thesis','supports','risks','monitor','beginner','additionalPositions'],additionalProperties:false};
...
// The AI may only PICK symbols that already exist in the deterministic
// rankedSignals list, and only supplies the bilingual rationale text; every
// other field (assetType, bias, score, confidence, coverage) is overwritten
// here from the deterministic packet itself, so a hallucinated ticker or a
// bias that disagrees with the engine can never reach the UI.
function buildPositionList(aiList,packet,excludeSymbol){
  const bySymbol=Object.fromEntries((packet.rankedSignals||[]).map(s=>[String(s.symbol||'').toUpperCase(),s]));
  const seen=new Set();const out=[];
  for(const item of Array.isArray(aiList)?aiList:[]){
    const symbol=String(item?.symbol||'').toUpperCase().trim();
    const s=bySymbol[symbol];
    if(!s||seen.has(symbol)||symbol===excludeSymbol)continue;
    seen.add(symbol);
    out.push({symbol,assetType:s.assetType,sector:s.sector,bias:s.bias,score:s.score,confidence:s.confidence,coverage:s.coverage,signalQuality:s.signalQuality,rationale:langObj(item.rationale)});
  }
...
app.post('/api/ai/intraday-report',async(req,res)=>{
  try{
    ...
    let synthesis={headline:langObj(primary?.thesis),topSignal:top.symbol,thesis:langObj(primary?.thesis),supports:langArr(primary?.supports),risks:langArr(second?.risks||primary?.caveats),monitor:langArr(second?.tests),beginner:langObj(primary?.beginner),additionalPositions:buildPositionList(primary?.additionalPositions,packet,top.symbol)};
    if(primary&&second&&process.env.ANTHROPIC_API_KEY&&process.env.ANTHROPIC_MODEL){
      try{
        ...
        synthesis={headline:langObj(syn.headline),topSignal:String(syn.topSignal||top.symbol),thesis:langObj(syn.thesis),supports:langArr(syn.supports),risks:langArr(syn.risks),monitor:langArr(syn.monitor),beginner:langObj(syn.beginner),additionalPositions:buildPositionList(syn.additionalPositions?.length?syn.additionalPositions:primary?.additionalPositions,packet,String(syn.topSignal||top.symbol))};
