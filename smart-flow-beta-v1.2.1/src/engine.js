export const ENGINE_VERSION='smart-v1.2.1-beta';
...
const ETF_META={
 ...
 IBIT:{type:'ETF',underlying:'Bitcoin',multiplier:1,sector:'Crypto'},FBTC:{type:'ETF',underlying:'Bitcoin',multiplier:1,sector:'Crypto'},BITO:{type:'ETF',underlying:'Bitcoin Futures',multiplier:1,sector:'Crypto'}
};
...
function canonical(r){
 ...
 const t={
   row:r._row,symbol,type,side:tradeSide,tx:tx(code),strike:num(field(r,'strike')),expiry:field(r,'expiry').slice(0,10),dte,
   ...
   price:tradePrice??0,bid:num(field(r,'bid')),ask:num(field(r,'ask')),time:field(r,'time'),code,tradeId:field(r,'tradeId'),assetTypeHint:field(r,'assetType'),condition:field(r,'condition')
 };
...
function annotateDuplicates(trades){const seen=new Map();return trades.map(t=>{const k=fingerprint(t);if(!k)return{...t,duplicate:false,duplicateOf:null};const duplicate=seen.has(k);if(!duplicate)seen.set(k,t.row);return{...t,duplicate,duplicateOf:duplicate?seen.get(k):null};});}
...
function annotateMultiLeg(trades){
 ...
 for(const arr of groups.values()){
   ...
   for(const t of arr)legInfo.set(t.row,{group:`ML-${gid}`,strategy,legs:arr.length,groupAmbiguity:ambiguity});gid++;
 }
 return trades.map(t=>({...t,...(legInfo.get(t.row)||{group:null,strategy:'SINGLE',legs:1,groupAmbiguity:'NONE'})}));
}
...
function resolveAsset(symbol,list){
 const hint=String(list.find(t=>t.assetTypeHint)?.assetTypeHint||'').toUpperCase();
 if(hint.includes('ETF')||hint.includes('FUND')) return ETF_META[symbol]||{type:'ETF',underlying:symbol,multiplier:1,sector:'Unknown'};
 return ETF_META[symbol]||{type:'STOCK',underlying:symbol,multiplier:1,sector:'Unknown'};
}
...
export function analyzeRows(rows){
 ...
 for(const [symbol,list] of map){
   ...
   const legacy=legacyFor(list),asset=resolveAsset(symbol,clean),bdown=breakdown(clean,eligiblePrem),quality=tickerDataQuality(clean),groups=groupSummaries(clean);
